 ...
 * CONSTANTS & MACROS
 */
#define SBP_PERIODIC_EVT_PERIOD              20     // 40Hz (25ms)
...
// =================== ??????????? ===================
#define PIN_FSR_POWER       GPIO_Pin_18 // ??????????????
#define PIN_MUX_1314        GPIO_Pin_11 // PA11 ???? ADC13/14
#define PIN_MUX_1516        GPIO_Pin_10 // PA10 ???? ADC15/16
...
static uint8_t IIC_Read_Byte(uint8_t ack) {
    ...
    SDA_OUT();
    if(ack) SDA_LOW(); else SDA_HIGH(); // ???? ACK ?? NACK
    SCL_HIGH(); IIC_Delay();
    SCL_LOW(); IIC_Delay();
    return receive;
}
...
void BMI270_Hardware_Init(void) {
    ...
    GPIOB_ModeCfg( GPIO_Pin_11, GPIO_ModeIN_PU );               // PB11(INT - ???????????????ж?)
    ...
    bmi270_dev.read_write_len = 32; // I2C burst ????
    ...
    if(rslt == BMI2_OK) {
        ...
        config[0].cfg.acc.odr = BMI2_ACC_ODR_100HZ;     // 100Hz ?????????? 40Hz ????
...
static uint16_t sample_channel_robust(uint8_t ch) {
    ...
    return (uint16_t)((sum >> 2) + RoughCalib_Value); // ???? 4 ??????????У??
}
...
void Peripheral_Init()
{
    ...
    // ? ? ??????????? PB0 (PWM6) ?????????
    // =======================================================
    GPIOB_ModeCfg(GPIO_Pin_0, GPIO_ModeOut_PP_5mA); // ????PB0????????
    PWMX_CLKCfg(4);                  // PWM??? = 60MHz / 4 = 15MHz
    PWMX_CycleCfg(PWMX_Cycle_256);   // ????256??PWM???? 58.5kHz?????????
    PWM6_ActDataWidth(255);          // ???????? (????????????????????)
    R8_PWM_OUT_EN |= RB_PWM6_OUT_EN; // ???? PWM6 ??? (? ?????????????)
    // =======================================================
...
uint16_t Peripheral_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    {
        static uint8_t breath_dir = 0; // 0=??(??????), 1=????(???????)
        static int16_t breath_val = 255; 

        if(peripheralConnList.connHandle == GAP_CONNHANDLE_INIT) 
        {
            // --- δ?????????????? 1.1V ---
            // ?????????1.1V????ζ?????????? 1.1V/3.3V = 1/3 (????? 85/256)??
            // (???: ?????????????????? 85 ??? 180 ?? 200??????????255???)
...
// 定时执行任务 (处理并发送 BMI270 和传感器数据)
// 定时执行任务 (处理并发送 BMI270 和传感器数据)
static void performPeriodicTask(void)
{
    // 1. 采集并计算 16 路 ADC 原始值
    FSR_Sample_All();
    ...
    // ================== 右脚专属点位映射 ==================
    static const uint8_t TX_SENSOR_MAP[16] = {
        1, 5, 4, 0, 15, 10, 11, 14, 
        8, 7, 13, 2, 6, 12, 3, 9
    };
    ...
    // 4. 填充 ADC 数据 (应用右脚映射并减去底噪 1530)
    for(int i = 0; i < 16; i++) {
        packet.adc[i] = (int16_t)(current_adc_values[TX_SENSOR_MAP[i]] - 1530);
    }
    
    // 5. 填充 IMU 数据 (直接装填，保持原始精度)
    ...
    // 6. 将 44 字节纯净数据直接射入蓝牙协议栈！
    peripheralChar4Notify((uint8_t *)&packet, sizeof(SensorDataPacket_t));
    
    // 7. 串口监视 (80Hz 下每秒只打印一次，防止堵塞)
    static uint8_t print_cnt = 0;
    if(++print_cnt >= 80) { 
        PRINT("Sent 80 Binary Pkts (Right Foot)! ACC_X: %d\r\n", packet.imu[0]);
        print_cnt = 0;
    }
}
